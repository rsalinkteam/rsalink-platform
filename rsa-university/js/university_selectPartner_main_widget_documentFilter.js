/*
Website:        RSA Select Partner Training (https://community.rsa.com/community/training/select-partner)
Iteration:      RSA University Redesign July 2019
Scope:          RSA Select Partner Training space
...
*/
        ...
        app.controller('ngApp', function($scope) {
            ...
            $scope.levelText = {
             ...
             $scope.init = function () {
                $scope.getProductRowData = jive_widget_container_large.find(".masterDocument tbody tr td:nth-child(1)").filter(function() {return $(this).text().toLowerCase().split(/[\s,]+/).includes("partners") == true}).parent('tr');
                jive_widget_container_large.find(".masterDocument tbody tr td:nth-child(1)").filter(function() {return $(this).text().toLowerCase().split(/[\s,]+/).includes("partners") == true}).parent('tr').find("a").attr("target", "_top");
                ...
                jive_widget_container_large.find("#rSAUniversotyQuestionsIssuesContact").parents(".jive-widget-htmltextwidget").css("margin-bottom","-15px");
