/*
Website:        RSA Adaptive Authentication Training (https://community.rsa.com/community/training/adaptive-authentication)
Iteration:      RSA University Redesign July 2019
Scope:          RSA Adaptive Authentication Training
...
*/
        ...
        app.controller('ngApp', function($scope) {
            ...
            $scope.init = function () {                
                $scope.getProductRowData = jive_widget_container_large.find(".masterDocument tbody tr td:nth-child(1)").filter(function() {return $(this).text().toLowerCase().split(/[\s,]+/).includes("customers") == true}).parent('tr').filter(function() {return $(this).find("td:nth-child(2)").text().toLowerCase() == "aa";});
                jive_widget_container_large.find(".masterDocument tbody tr td:nth-child(1)").filter(function() {return $(this).text().toLowerCase().split(/[\s,]+/).includes("customers") == true}).parent('tr').filter(function() {return $(this).find("td:nth-child(2)").text().toLowerCase() == "aa";}).find("a").attr("target", "_top");
                ...
                $scope.getCourseRowData = $scope.getProductRowData;file:///C:/Users/rajakc/AppData/Local/slack/app-4.0.2/resources/app.asar/dist/notifications.html#
