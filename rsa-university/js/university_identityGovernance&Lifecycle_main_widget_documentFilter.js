/*
Website:        RSA Identity Governance & Lifecycle Training (https://community.rsa.com/community/training/governance-and-lifecycle)
Iteration:      RSA University Redesign July 2019
Scope:          RSA Identity Governance & Lifecycle Training space
...
*/
        ...
        app.controller('ngApp', function($scope) {
            ...
            $scope.init = function () {                
                $scope.getProductRowData = jive_widget_container_large.find(".masterDocument tbody tr td:nth-child(1)").filter(function() {return $(this).text().toLowerCase().split(/[\s,]+/).includes("customers") == true}).parent('tr').filter(function() {return $(this).find("td:nth-child(2)").text().toLowerCase() == "igl";});
                jive_widget_container_large.find(".masterDocument tbody tr td:nth-child(1)").filter(function() {return $(this).text().toLowerCase().split(/[\s,]+/).includes("customers") == true}).parent('tr').filter(function() {return $(this).find("td:nth-child(2)").text().toLowerCase() == "igl";}).find("a").attr("target", "_top");
