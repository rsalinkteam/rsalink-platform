/*
Website:        RSA SecurID Suite Training (https://community.rsa.com/community/training/securid)
Iteration:      RSA University Redesign July 2019
Scope:          RSA SecurID Suite Training space
...
*/
        ...
        app.controller('ngApp', function($scope) {
            ...
            $scope.init = function () {                
                $scope.getProductRowData = jive_widget_container_large.find(".masterDocument tbody tr td:nth-child(1)").filter(function() {return $(this).text().toLowerCase().split(/[\s,]+/).includes("customers") == true}).parent('tr').filter(function() {return $(this).find("td:nth-child(2)").text().toLowerCase() == "sida";});
                jive_widget_container_large.find(".masterDocument tbody tr td:nth-child(1)").filter(function() {return $(this).text().toLowerCase().split(/[\s,]+/).includes("customers") == true}).parent('tr').filter(function() {return $(this).find("td:nth-child(2)").text().toLowerCase() == "sida";}).find("a").attr("target", "_top");
